...
c+" ('"+b.END+"' expected)");return a};c.prototype._parseExtend=function(b,a){var c=this.tn.stack.slice(),e=this.tn.index;a=this.tn.next();this.tn.stack=c;this.tn.index=e;this._parseIgnored(b,"extend");return a};c.prototype._parseOption=function(c,a){a=this.tn.next();var d=!1;a==b.COPTOPEN&&(d=!0,a=this.tn.next());if(!b.NAME.test(a))throw Error("Illegal option name in message "+c.name+": "+a);var e=a;a=this.tn.next();if(d){if(a!=b.COPTCLOSE)throw Error("Illegal custom option name delimiter in message "+
c.name+", option "+e+": "+a+" ('"+b.COPTCLOSE+"' expected)");a=this.tn.next()}if(a!=b.EQUAL)throw Error("Illegal option operator in message "+c.name+", option "+e+": "+a+" ('"+b.EQUAL+"' expected)");a=this.tn.next();if(a==b.STRINGOPEN){if(d=this.tn.next(),a=this.tn.next(),a!=b.STRINGCLOSE)throw Error("Illegal end of option value in message "+c.name+", option "+e+": "+a+" ('"+b.STRINGCLOSE+"' expected)");}else if(b.NUMBER.test(a))d=parseInt(a);else if(b.NAME.test(a))d=a;else throw Error("Illegal option value in message "+
c.name+", option "+e+": "+a);a=this.tn.next();if(a!=b.END)throw Error("Illegal end of option in message "+c.name+", option "+e+": "+a+" ('"+b.END+"' expected)");c.options[e]=d};c.prototype._parseIgnored=function(c,a){var d=this.tn.next();if(!b.TYPEREF.test(d))throw Error("Illegal "+a+" type in "+c.name+": "+d);var e=d,d=this.tn.next();if(d!=b.OPEN)throw Error("Illegal OPEN in "+c.name+" after "+a+" "+e+": "+d);var g=1;do{d=this.tn.next();if(null===d)throw Error("Illegal nesting in "+c.name+", "+a+
" "+e+": EOF");if(d==b.OPEN)g++;else if(d==b.CLOSE&&(g--,0==g))break}while(1)};c.prototype._parseMessage=function(c,a){var d={};a=this.tn.next();if(!b.NAME.test(a))throw Error("Illegal message name"+(c?" in message "+c.name:"")+": "+a);d.name=a;a=this.tn.next();if(a!=b.OPEN)throw Error("Illegal OPEN after message "+d.name+": "+a+" ('"+b.OPEN+"' expected)");d.fields=[];d.enums=[];d.messages=[];d.options={};do if(a=this.tn.next(),a==b.CLOSE)break;else if(b.RULE.test(a))this._parseMessageField(d,a);
else if("enum"==a)this._parseEnum(d,a);else if("message"==a)this._parseMessage(d,a);else if("option"==a)this._parseOption(d,a);else throw Error("Illegal token in message "+d.name+": "+a+" (type or '"+b.CLOSE+"' expected)");while(1);c.messages.push(d);return d};c.prototype._parseMessageField=function(c,a){var d={};d.rule=a;a=this.tn.next();if(!b.TYPE.test(a)&&!b.TYPEREF.test(a))throw Error("Illegal field type in message "+c.name+": "+a);d.type=a;a=this.tn.next();if(!b.NAME.test(a))throw Error("Illegal field name in message "+
c.name+": "+a);d.name=a;a=this.tn.next();if(a!=b.EQUAL)throw Error("Illegal field number operator in message "+c.name+"#"+d.name+": "+a+" ('"+b.EQUAL+"' expected)");a=this.tn.next();if(!b.ID.test(a))throw Error("Illegal field number in message "+c.name+"#"+d.name+": "+a);d.id=parseInt(a,10);d.options={};a=this.tn.next();a==b.OPTOPEN&&(this._parseFieldOptions(c,d,a),a=this.tn.next());if(a!=b.END)throw Error("Illegal field delimiter in message "+c.name+"#"+d.name+": "+a+" ('"+b.END+"' expected)");c.fields.push(d)};
c.prototype._parseFieldOptions=function(c,a,d){var e=!0;do{d=this.tn.next();if(d==b.OPTCLOSE)break;else if(d==b.OPTEND){if(e)throw Error("Illegal start of message field options in message "+c.name+"#"+a.name+": "+d);d=this.tn.next()}this._parseFieldOption(c,a,d);e=!1}while(1)};c.prototype._parseFieldOption=function(c,a,d){var e=!1;d==b.COPTOPEN&&(d=this.tn.next(),e=!0);if(!b.NAME.test(d))throw Error("Illegal field option in message "+c.name+"#"+a.name+": "+d);var g=d;d=this.tn.next();if(e){if(d!=
b.COPTCLOSE)throw Error("Illegal custom field option name delimiter in message "+c.name+"#"+a.name+": "+d+" (')' expected)");d=this.tn.next()}if(d!=b.EQUAL)throw Error("Illegal field option operation in message "+c.name+"#"+a.name+": "+d+" ('=' expected)");d=this.tn.next();if(d==b.STRINGOPEN){if(e=this.tn.next(),d=this.tn.next(),d!=b.STRINGCLOSE)throw Error("Illegal end of field value in message "+c.name+"#"+a.name+", option "+g+": "+d+" ('"+b.STRINGCLOSE+"' expected)");}else if(b.NUMBER.test(d))e=
parseFloat(d);else if(b.TYPEREF.test(d))e=d;else throw Error("Illegal field option value in message "+c.name+"#"+a.name+", option "+g+": "+d);a.options[g]=e};c.prototype._parseEnum=function(c,a){var d={};a=this.tn.next();if(!b.NAME.test(a))throw Error("Illegal enum name in message "+c.name+": "+a);d.name=a;a=this.tn.next();if(a!=b.OPEN)throw Error("Illegal OPEN after enum "+d.name+": "+a);d.values=[];d.options={};do{a=this.tn.next();if(a==b.CLOSE)break;if("option"==a)this._parseOption(d,a);else{if(!b.NAME.test(a))throw Error("Illegal enum value name in enum "+
d.name+": "+a);this._parseEnumValue(d,a)}}while(1);c.enums.push(d)};c.prototype._parseEnumValue=function(c,a){var d={};d.name=a;a=this.tn.next();if(a!=b.EQUAL)throw Error("Illegal enum value operator in enum "+c.name+": "+a+" ('"+b.EQUAL+"' expected)");a=this.tn.next();if(!b.ID.test(a))throw Error("Illegal enum value value in enum "+c.name+": "+a);d.id=parseInt(a,10);c.values.push(d);a=this.tn.next();a==b.OPTOPEN&&(this._parseFieldOptions(c,{options:{}},a),a=this.tn.next());if(a!=b.END)throw Error("Illegal enum value delimiter in enum "+
...
a.prototype.build=function(){for(var d=function(a,b){var d=b.getChildren(a.Message.Field),g=eval("(function "+b.name+"() { this.__construct.apply(this, arguments); })");g.prototype.__construct=function(a){var b,g;for(b=0;b<d.length;b++)g=d[b],this[g.name]=g.repeated?[]:null;for(b=0;b<d.length;b++)if(g=d[b],"undefined"!=typeof g.options["default"])try{this.set(g.name,g.options["default"])}catch(k){throw Error("[INTERNAL ERROR] "+k);}if(1==arguments.length&&"object"==typeof a&&"function"!=typeof a.encode&&
...
null===this[g.name]&&(this[g.name]=[]);this[g.name].push(g.verifyValue(d,!0))};g.prototype.set=function(c,d){var g=b.getChild(c);if(!g)throw Error(this+"#"+c+" is not a field: undefined");if(!(g instanceof a.Message.Field))throw Error(this+"#"+c+" is not a field: "+g.toString(!0));this[g.name]=g.verifyValue(d)};g.prototype.get=function(c){var g=b.getChild(c);if(!g||!(g instanceof a.Message.Field))throw Error(this+"#"+c+" is not a field: undefined");if(!(g instanceof a.Message.Field))throw Error(this+
"#"+c+" is not a field: "+g.toString(!0));return this[g.name]};for(var k=0;k<d.length;k++)(function(a){var c=a.name.replace(/(_[a-zA-Z])/g,function(a){return a.toUpperCase().replace("_","")}),c=c.substring(0,1).toUpperCase()+c.substring(1),d=a.name.replace(/([A-Z])/g,function(a){return"_"+a});b.hasChild("set"+c)||(g.prototype["set"+c]=function(b){this.set(a.name,b)});b.hasChild("set_"+d)||(g.prototype["set_"+d]=function(b){this.set(a.name,b)});b.hasChild("get"+c)||(g.prototype["get"+c]=function(){return this.get(a.name)});
b.hasChild("get_"+d)||(g.prototype["get_"+d]=function(){return this.get(a.name)})})(d[k]);g.prototype.encode=function(a){a=a||new n;var c=a.littleEndian,g=b.encode(this,a.LE()).flip();a.littleEndian=c;return g};g.prototype.toArrayBuffer=function(){return this.encode().toArrayBuffer()};g.decode=function(a){a=a?a instanceof n?a:n.wrap(a):new n;var c=a.littleEndian,g=b.decode(a.LE());a.littleEndian=c;return g};g.prototype.toString=function(){return b.toString()};Object.defineProperty&&Object.defineProperty(g,
"$options",{value:b.buildOpt(),enumerable:!1,configurable:!1,writable:!1});return g}(b,this),k=this.getChildren(),l=0;l<k.length;l++)if(k[l]instanceof e)d[k[l].name]=k[l].build();else if(k[l]instanceof a)d[k[l].name]=k[l].build();else if(!(k[l]instanceof a.Field))throw Error("Illegal reflect child of "+this.toString(!0)+": "+k[l].toString(!0));return this.built=d};a.prototype.encode=function(b,c){for(var d=this.getChildren(a.Field),e=0;e<d.length;e++)d[e].encode(b.get(d[e].name),c);return c};a.prototype.decode=
...
b.Message=a;var d=function(a,b,c,d,e,h){f.call(this,a,d);this.required="required"==b;this.repeated="repeated"==b;this.type=c;this.id=e;this.options=h||{}};d.prototype=Object.create(f.prototype);d.prototype.verifyValue=function(a,b){b=b||!1;if(null===a){if(this.required)throw Error("Illegal value for "+this.toString(!0)+": "+a+" (required)");return null}var d,f;if(this.repeated&&!b){a instanceof Array||(a=[a]);f=[];for(d=0;d<a.length;d++)f.push(this.verifyValue(a[d],!0));return f}if(!this.repeated&&
...
e.fields.length;l++){if(!h.isValidMessageField(e.fields[l]))throw Error("Not a valid message field definition in message "+g.name+": "+JSON.stringify(e.fields[l]));if(g.hasChild(e.fields[l].id))throw Error("Duplicate field id in message "+g.name+": "+e.fields[l].id);if(e.fields[l].options){k=Object.keys(e.fields[l].options);for(n=0;n<k.length;n++){if(!b.NAME.test(k[n]))throw Error("Illegal field option name in message "+g.name+"#"+e.fields[l].name+": "+k[n]);if(!b.ID.test(""+e.fields[l].options[k[n]])&&
...
k.length&&(c.push(a),a=k,this.ptr=g)}else if(h.isValidEnum(e)){g=new f.Enum(this.ptr,e.name,e.options);for(l=0;l<e.values.length;l++)g.addChild(new f.Enum.Value(g,e.values[l].name,e.values[l].id));this.ptr.addChild(g)}else throw Error("Not a valid message or enum definition: "+JSON.stringify(e));else throw Error("Not a valid namespace definition: "+JSON.stringify(a));this.ptr=this.ptr.parent}this.resolved=!1;this.result=null;return this}};h.prototype["import"]=function(a,b){a["package"]&&this.define(a["package"],
a.options);a.messages&&this.create(a.messages);this.reset();a["package"]&&this.define(a["package"],a.options);a.enums&&this.create(a.enums);this.reset();if(a.imports&&0<a.imports.length){if(!b)throw Error("Cannot determine import root: File name is unknown");for(var e=b.replace(/[\/\\][^\/\\]*$/,""),g=0;g<a.imports.length;g++){var f=e+"/"+a.imports[g];if(/\.json$/i.test(f)){var h=c.Util.fetch(f);if(null===h)throw Error("Failed to import '"+f+"' in '"+b+"': File not found");this["import"](JSON.parse(h),
f)}else if(!/google\/protobuf\//.test(f)){h=c.Util.fetch(f);if(null===h)throw Error("Failed to import '"+f+"' in '"+b+"': File not found");h=new c.DotProto.Parser(h+"");this["import"](h.parse(),f)}}}return this};h.prototype.resolveAll=function(){if(!(null==this.ptr||"object"==typeof this.ptr.type)){if(this.ptr instanceof f.Namespace)for(var a=this.ptr.getChildren(),d=0;d<a.length;d++)this.ptr=a[d],this.resolveAll();else if(this.ptr instanceof f.Message.Field)if(b.TYPE.test(this.ptr.type))this.ptr.type=
